# from: https://github.com/xinntao/ESRGAN/blob/master/RRDBNet_arch.py
...
class RRDBNet(nn.Module):
    def __init__(self, in_nc, cond_channels, nb, s, input_shape, gc=32):
        ...
        #### upsampling
        self.upconv1 = nn.Conv2d(cond_channels, cond_channels, 3, 1, 1, bias=True)
        self.upconv2 = nn.Conv2d(cond_channels, cond_channels, 3, 1, 1, bias=True)
        self.HRconv = nn.Conv2d(cond_channels, cond_channels, 3, 1, 1, bias=True)
        self.conv_last = nn.Conv2d(cond_channels, cond_channels, 3, 1, 1, bias=True)

        self.lrelu = nn.LeakyReLU(negative_slope=0.2, inplace=True)

    def forward(self, x):
        # print(f"RRDBNet::forward(): {x}")
